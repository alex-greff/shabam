...
/**
 * Copies over the audio buffer data from the given window index for the given duration.
 * 
 ...
 */
function copyWindow(source: AudioBuffer, target: AudioBuffer, windowIndex: number, windowDuration: number) {
    // Calculate the start index from where the copy will take place
    const startIndex = Math.floor(source.sampleRate * windowIndex * windowDuration);
 ...
 */
export async function downsample(source: AudioBuffer, targetSampleRate = AudioConstants.TARGET_SAMPLE_RATE) {
    const audioContext = new OfflineAudioContext(
    ...
    const sourceNode = new AudioBufferSourceNode(audioContext, { buffer: source });
    sourceNode.connect(audioContext.destination);
    sourceNode.start(0);
...
/**
 * Computes the partition ranges for the given FFT size in respect to the number of partitions needed.
 * Returns an array of tuples (2 element arrays) of the computed partition ranges.
 * 
...
export function computePartitionRanges(
    ...
    function getBoundaryIndex(partitionIdx: number, totalPartitions: number, totalBins: number) {
        // --- Checks ---
        if (totalBins <= 0)
    ...
    for (let partitionIdx of range) {
        ...
        const max = getBoundaryIndex(partitionIdx + 1, partitionAmount, FFTSize/2);
...
export async function computeSpectrogramData(
    ...
    const optionsNormalized: ComputeSpectrogramDataOptions = { ...defaultOptions, ...options };
    ...
    for (let currWindow = 0; currWindow < numWindows; currWindow++) {
        const { frequencyData } = await computeFFTData(source, "frequency", currWindow, windowDuration, FFTSize, windowSmoothing);
