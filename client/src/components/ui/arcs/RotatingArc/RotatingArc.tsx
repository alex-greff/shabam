...
import BaseArc, {
  Props as BaseArcProps,
} from "@/components/ui/arcs/BaseArc/BaseArc";
...
const RotatingArc: FunctionComponent<Props> = (props) => {
  ...
  const [rotationTween, setRotationTween] = useState<gsap.core.Tween | null>(
    null
  );
  ...
  // Remake tween whenever one of the parameters changes 
  // (also sets it up initially)
  useEffect(() => {
  ...
  return (
    <div
      ...
      id={props.id}
    >
      <BaseArc {...rest} className="RotatingArc__base-arc" ref={baseRef} />
    </div>
  );
};
