...
const SearchScene: FunctionComponent<Props & AppRouteComponentProps> = (
  props
) => {
  ...
  const [isSearchView, setIsSearchView] = useState<boolean>(
    !!matchPath("/search", {
  ...
  useEffect(() => {
    ...
    const isHomeView = !!matchPath(currPathName, {
      path: "/",
      exact: true,
      strict: false,
    });
    const isSearchView = !!matchPath(currPathName, {
      path: "/search",
      exact: true,
      strict: false,
    });
  ...
  return (
    <div
      className={classnames("SearchScene", props.className, {
        "is-search-view": isSearchView,
      })}
      style={props.style}
      id="SearchScene"
    >
      <div className="SearchScene__scene" ref={sceneRef}>
        <div className={"SearchScene__main-circle"}>
          <div className="SearchScene__main-circle-s">S</div>
        </div>

        <div className="SearchScene__arcs-container">
          {ARCS.map((arc, num) => (
            <div className="SearchScene__arc-container" key={`arc-num-${num}`}>
              <RotatingArc
...
SearchScene.defaultProps = {} as Partial<Props>;
