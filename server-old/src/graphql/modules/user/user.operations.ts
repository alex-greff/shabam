...
export default {
  login: async (
    ...
    context.res.setHeader(
      "Set-Cookie",
      cookie.serialize(USERNAME_COOKIE_NAME, userUsername, {
        path: "/",
  ...
  logout: async (
    ...
    context.res.setHeader(
      "Set-Cookie",
      cookie.serialize(USERNAME_COOKIE_NAME, "", {
        path: "/",
        maxAge: -1,
        httpOnly: false,
        secure: KEYS.PRODUCTION ? true : false,
  ...
  checkAvailability: async (
    root: any,
    { username }: CheckUsernameAvailabilityArgs,
    context: AppContext
  ): Promise<boolean> => {
  ...
  editUser: async (
    root: any,
    { username: currUsername, updatedCredentials }: EditUserArgs,
    context: AppContext
  ): Promise<boolean> => {
  ...
  editUserRole: async (
    root: any,
    { username, updatedRole }: EditUserRoleArgs,
    context: any
  ): Promise<boolean> => {
    // Check that the user exists
    const userExists = await helpers.userExists(username);
