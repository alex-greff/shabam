  ...
  return async (
    ...
    const canDoOperation = async (role: string, operation: string) => {
      ...
      const inheritChecks = $role.inherits.map((inheritedRole: any) => {
        return () =>
          Utilities.booleanResolver(canDoOperation, inheritedRole, operation);
      });
      return await Utilities.resolveAsBoolean(() =>
        Utilities.PromiseUtilities.some(inheritChecks)
      );
    };
    ...
    return await Utilities.resolveAsBoolean(() =>
      Utilities.PromiseUtilities.some(operationChecks)
    );
  };
};
