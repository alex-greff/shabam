...
export async function getTrack(trackID: number): Promise<Track> {
  ...
  const dbQueries = [
    db.query(trackQuery, trackID),
    db.query(artistsQuery, trackID),
  ];
  ...
  if (resArtists.rowCount > 0) {
    aArtists = resArtists.rows.map(
      (i_oCurrArtist: any) => i_oCurrArtist.artist_name
    );
  }
...
export async function getAllTracks(): Promise<Track[]> {
  ...
  const dbQueries = [
    db.query(allTracksQuery),
    db.query(trackToArtistsMapQuery),
  ];
  const [allTracksRes, trackToArtistsMapRes] = await Promise.all(dbQueries);

  const trackToArtistsMap = trackToArtistsMapRes.rows.reduce(
    (acc: any, oCurrRawMap: any) => {
  ...
  const rawTrackData = allTracksRes.rows.map((oTrack: any) => ({
    ...oTrack,
    artists: trackToArtistsMap[oTrack.track_id]
      ? [...trackToArtistsMap[oTrack.track_id]]
      : [],
  }));
...
export async function addTrack(
  ...
  const insertTrackRes = await db.query(
    insertTrackQuery,
    username,
    ...insertArgs
  );
...
export async function editTrack(
  ...
  if (updateArgs.length <= 0 && !updateArtists) {
    throw new Error(
      "Error updating track, at least one field must be provided to update"
    );
  }
