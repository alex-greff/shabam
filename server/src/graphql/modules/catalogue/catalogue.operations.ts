...
export default {
  ...
  searchTrack: async (
    root: any,
    { fingerprint, fingerprintInfo }: SearchTrackArgs,
    context: AppContext
  ): Promise<any> => {
    // TODO: fix return typedef
    ...
    const fingerprintBuffer = await FingerprintUtilities.getFingerprintBuffer(
      createReadStream
    );
    let fingerprintData = FingerprintUtilities.getFingerprintData(
      fingerprintBuffer
    );
    ...
    const endpointURL =
      "http://identification-worker:5002/identify_fingerprint";
  ...
  addTrack: async (
    root: any,
    { fingerprint, trackData }: AddTrackArgs,
    context: AppContext
  ): Promise<Track> => {
    ...
    const trackID = await helpers.addTrack(
      title,
      artists,
      coverImage!,
      releaseDate,
  ...
  editTrack: async (
    root: any,
    { trackID, updatedTrackData }: EditTrackArgs,
    context: AppContext
  ): Promise<Track> => {
    const {
      title: newTitle,
  ...
  recomputeTrackFingerprint: async (
    root: any,
    { trackID, fingerprint }: RecomputeTrackFingerprintArgs,
    context: AppContext
  ): Promise<any> => {
    // TODO: complete return typeDef
    // TODO: complete
  },
};
